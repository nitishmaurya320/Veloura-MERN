...
const Navbar = () => {
  ...
  return (
    <>
        ...
        <div className="w-[150px]  h-[50%] mx-5  md:w-[250px] md:h-[70%] ">
          <Link to="/">
            <img
              src="/logo.png"
              alt="logo"
              className="w-full h-full hover:scale-103"
            />
          </Link>
        </div>
        <div>
      ...
      <div
        className={`fixed h-screen  top-0 left-0 sm:w-1/2 md:w-1/4 w-3/5 z-1001  transition-transform  duration-100 bg-white ${showMenuBar ? "translate-x-0" : "translate-x-[-100%]"}`}
      >
        ...
        <ul className=" space-x-6 text-[20px]  p-5">
          {menuItems.map((item, index) => {
            return (
              <Link
                to={item.path}
                className="hover:text-blue-800 text-[15px]"
                key={index}
              >
                <li
                  onClick={() => {
                    window.scrollTo({ top: 0 }, handleMenubar());
                  }}
                >
                  {item.name}
                </li>
