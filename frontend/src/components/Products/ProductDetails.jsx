...
const ProductDetails = ({ productId }) => {
  ...
  useEffect(() => {
    if (productFetchId) dispatch(fetchProductDetails(productFetchId));
        ...
        <div className="md:max-w-[80%] w-[100%] mx-auto  flex flex-col md:flex-row h-full">
          <div className="flex-col-reverse flex  md:flex-row mx-auto  md:mx-0 md:w-[40%] ml-0 mr-0 w-full  md:space-y-4  md:gap-2 mt-5 md:mt-0">
              ...
              {selectedProduct.product.images.map((Product, index) => {
                return (
                  <div className="box-border" key={index}>
                    <img
                      className={`md:w-[80px] md:h-[80px] w-[70px] h-[70px] rounded-lg  box-content object-cover transition-all outline-2  ${index == currentProduct ? " outline-gray-600   " : "outline-transparent"}`}
                      onClick={() => {
                        handlecurrentProduct(index);
                      }}
                      src={Product.url}
                    />
                  </div>
            ...
            <div className="w-full flex   h-[500px]  md:h-[500px]">
              <img
                className="md:rounded-lg w-full h-full object-cover object-center"
                src={selectedProduct.product.images[`${currentProduct}`]?.url}
                alt="Main product    "
              />
            </div>
          </div>
          <div className="  w-full md:w-[50%] flex mt-2 md:mt-0 md:ml-5 px-4 ">
            <div className="md:text-[15px] text-[13px]">
              <div className="md:text-[20px] text-[18px]  font-semibold text-gray-600">
                {selectedProduct.product.name}
              </div>
              <div className="line-through  font-semibold text-gray-500">
                ₹
                {selectedProduct.product.originalPrice &&
                  selectedProduct.product.originalPrice}
              </div>
              <p className="flex font-semibold  items-center text-2xl">
              ...
              <div className="">
                ...
                <div className="space-x-2">
                  {selectedProduct.product.colors.map((color) => {
                    return (
                      <button
                        key={color}
                        className={`rounded-full w-[30px] h-[30px] border-3 outline outline-gray-200 p-3 ${color == selectedColor ? "border-3 border-blue-700" : "border-transparent"}`}
                        onClick={() => {
                ...
                <div className="space-x-3">
                  {selectedProduct.product.sizes.map((size) => {
                    return (
                      <button
                        key={size}
                        className={`border-2 px-2 ${size === selectedSize ? "border-2 border-blue-500 " : "border-gray-200"}`}
                ...
                <div className="flex justify-center">
                  <button
                    ...
                    className={`w-[90%] h-[40px]  text-white bg-[#9B2A90] hover:bg-[#7E1F75] rounded mt-4 ${isButtonDisabled ? "bg-black-600 " : "bg-[#9B2A90]"}`}
                  >
                  ...
                  <table className="w-full text-left">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="py-1">
                          {selectedProduct.product.material}
                        </td>
                      </tr>
                    </tbody>
                  </table>
